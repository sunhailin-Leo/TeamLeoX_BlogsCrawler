...
tf.app.flags.DEFINE_boolean(
    "restore", True, "whether to restore from the latest checkpoint"
)
...
class DataIterator:
    def __init__(self, data_dir):
        ...
        for root, sub_folder, file_list in os.walk(data_dir):
            for file_path in file_list:
                ...
                im = (
                    np.array(Image.open(image_name).convert("L")).astype(np.float32)
                    / 255.0
                )
    ...
    def input_index_generate_batch(self, index=None):
        if index:
            image_batch = [self.image[d] for d in index]
            label_batch = [self.labels[d] for d in index]
        else:
        ...
        def get_input_lens(sequences):
            ...
            # 所以这里的长度都是一样的
            lengths = np.asarray(
                [FLAGS.max_stepsize for _ in sequences], dtype=np.int64
            )
...
def accuracy_calculation(original_seq, decoded_seq, ignore_value=-1, is_print: bool = False):
    if len(original_seq) != len(decoded_seq):
        ...
        return 0
    count = 0
    for index, origin_label in enumerate(original_seq):
        decoded_label = [j for j in decoded_seq[index] if j != ignore_value]
        if is_print and index < maxPrintLen:
